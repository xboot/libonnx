...
static void ReduceL1_int8(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			sum += abs(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
...
static void ReduceL1_int32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			sum += labs(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = sum;
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceL1_int64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			sum += llabs(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = sum;
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceL1_uint8(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceL1_uint32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceL1_uint64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceL1_bfloat16(struct onnx_node_t * n)
{
	...
	do
	{
		...
		do
		{
			sum += fabsf(bfloat16_to_float32(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]));
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
...
static void ReduceL1_float16(struct onnx_node_t * n)
{
	...
	do
	{
		...
		do
		{
			sum += fabsf(float16_to_float32(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]));
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = float32_to_float16(sum);
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceL1_float32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			sum += fabsf(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = sum;
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceL1_float64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			sum += fabs(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = sum;
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}
