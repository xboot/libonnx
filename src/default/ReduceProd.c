...
static int ReduceProd_init(struct onnx_node_t * n)
{
...
static int ReduceProd_exit(struct onnx_node_t * n)
{
...
static int ReduceProd_reshape(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
...
static void ReduceProd_int8(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			prod *= px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)];
...
static void ReduceProd_int32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_int64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_uint8(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_uint32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_uint64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_bfloat16(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			prod *= bfloat16_to_float32(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = float32_to_bfloat16(prod);
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceProd_float16(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
	...
	do
	{
		...
		do
		{
			prod *= float16_to_float32(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
		} while(dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
		py[i++] = float32_to_float16(prod);
	} while(dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}

static void ReduceProd_float32(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_float64(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
void resolver_default_op_ReduceProd(struct onnx_node_t * n)
{
	...
	case ONNX_TENSOR_TYPE_INT8:
		...
		n->operator = ReduceProd_int8;
		break;
	case ONNX_TENSOR_TYPE_INT32:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_int32;
		break;
	case ONNX_TENSOR_TYPE_INT64:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_int64;
		break;
	case ONNX_TENSOR_TYPE_UINT8:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_uint8;
		break;
	case ONNX_TENSOR_TYPE_UINT32:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_uint32;
		break;
	case ONNX_TENSOR_TYPE_UINT64:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_uint64;
		break;
	case ONNX_TENSOR_TYPE_BFLOAT16:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_bfloat16;
		break;
	case ONNX_TENSOR_TYPE_FLOAT16:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_float16;
		break;
	case ONNX_TENSOR_TYPE_FLOAT32:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_float32;
		break;
	case ONNX_TENSOR_TYPE_FLOAT64:
		n->init = ReduceProd_init;
		n->exit = ReduceProd_exit;
		n->reshape = ReduceProd_reshape;
		n->operator = ReduceProd_float64;
		break;
	default:
		break;
	}
